(function( $ ) { 
	...
	var uploadNonce = opaljob.uploadNonce;
	var fileTypeTitle =  opaljob.fileTypeTitle ;  
    var ajaxURL = opaljob.ajaxURL;
    ...
    if( $("#select-images").length > 0 ){    
        ...
        var uploader = new plupload.Uploader({
            ...
            url: opaljob.ajaxURL + "?action=opaljob_upload_images&nonce=" + uploadNonce,
            filters: {
                mime_types : [
        ...
        $('.gallery-thumbnail').delegate('.icon-delete' ,'click', function() {
            ...
            $.ajax({
                type: "POST",
                url: ajaxurl,
                data: 'property_id='+$(this).data('property-id')+'&action=opaljob_delete_property_image&attachment_id='+$(this).data('attachment-id'), // serializes the form's elements.
                dataType:'json',
    ...
    if( $("#select-profile-image").length ){     
        var uploader = new plupload.Uploader({
            ...
            url: ajaxURL + "?action=opaljob_upload_user_avatar&nonce=" + uploadNonce,
            filters: {
                mime_types : [
                    { title : fileTypeTitle, extensions : "jpg,jpeg,gif,png" }
                ],
