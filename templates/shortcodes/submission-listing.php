			...
			<?php if( $loop->have_posts() ): ?> 	
				...
				<?php $cnt=0; while ( $loop->have_posts() ) : $loop->the_post(); global $post;  ?>
					<tr>
						...
						<td><?php the_ID(); ?></td>
						<td><a href="<?php echo opaljob_submission_edit( array( 'id' => $post->ID ) ); ?>"><?php the_title(); ?></a></td> 
						<td><span class="status-<?php echo get_post_status(); ?>"><?php echo get_post_status(); ?></span></td> 
						<td>
							<a href="<?php echo opaljob_submission_edit( array( 'id' => $post->ID ) ); ?>"><?php _e( 'Edit', 'opaljob' ); ?></a>
							<a href="<?php echo opaljob_submission_delete( array('remove'=>'delete', 'id' => $post->ID , 'page' => 'submission_list_page' ) ); ?>"><?php _e( 'Delete', 'opaljob' ); ?></a>
						</td> 
