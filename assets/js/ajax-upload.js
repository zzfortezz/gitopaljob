jQuery(document).ready(function($) {
    ...
    if (typeof(plupload) !== 'undefined') {
        ...
        var nonce = $(".opal_uploader").data('nonce');
        //config for plupload
        var config = {
            ...
            filters: {
                mime_types: [{
                    title: 'Image Files',
                    extensions: 'pdf,jpg,jpeg,png,ico'
                }]
            },
            multipart_params: {
                fileID: uploaderID,
        ...
        uploader.bind('FileUploaded', function (up, file, res) {
            $("#" + uploaderID + "_container").unblock();
            var result = $.parseJSON(res.response); 
            var data = result.data;
            if(result.success){ 
                if(data.type == "pdf"){
                    $(".result_append").html(data.title + '<i class="fa fa-times button_remove_file" data-id='+ data.id +'></i>');
                }
            }else{
