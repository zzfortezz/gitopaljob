...
class Opalestate_Field_Map {
		...
		echo '<div class="row">
			...
			<div class="col-sm-6">
					<div  class="form-group">
						<label>'.__( 'Map Address', 'opaljob' ).'</label>
				...
				$field_type_object->_desc( true, true );
					...
					echo '<label>'.__( 'Latitude', 'opaljob' ).'</label>';
					...
					echo '</div>';
					echo ' <div class="form-group">';
					echo '<label>'.__( 'Longitude',  'opaljob' ).'</label>';
					echo $field_type_object->input( array(
						'type'       => 'text',
	...
	public static function setup_admin_scripts() {
		...
		$api = apply_filters( 'opaljob_google_map_api_uri',  '//maps.googleapis.com/maps/api/js?sensor=false&amp;libraries=places&amp;key='.$key   );
		wp_enqueue_script("opaljob-google-maps", $api, null, "0.0.1", false);

		wp_enqueue_script( 'opaljob-google-maps-js', plugins_url( 'js/script.js', __FILE__ ), array(  ), self::VERSION );
		wp_enqueue_style( 'opaljob-google-maps', plugins_url( 'css/style.css', __FILE__ ), array(), self::VERSION );
